...
get_growth_data = function(
    ...
    # Before loop
    pos_g = vector(mode = "list", length = num_replicates_1)
    for (my_rep in seq_len(num_replicates_1)) {
        pos_g[[my_rep]] = drop_cols_with_negs(X = g[ , , my_rep])
    }
...
```{r}
sum_var_logs = vector(mode = "numeric", length = NUM_REPLICATES_1)
for (my_rep in seq_along(try_1)) {
    sum_var_logs[my_rep] = get_sum_var_logs(try_1[[my_rep]])
}
sum_var_logs
```
